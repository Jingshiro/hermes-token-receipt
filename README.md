...
在聊天中直接输入以下命令，即可将新笑话添加到笑话库：

```text
/receipt joke 你知道吗？写一个 README 比写代码多消耗 {n} token。
```

新笑话会自动保存到 `jokes.yaml`，重启 Gateway 后生效。

### 手动编辑

你也可以直接编辑插件目录下的 `jokes.yaml` 文件，按 YAML 格式添加笑话。记得在笑话中使用 `{n}` 作为随机 token 数字的占位符喵！
